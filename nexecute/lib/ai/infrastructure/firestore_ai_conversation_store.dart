...
class FirestoreAiConversationStore implements AiConversationStore {
  ...
  Future<void> saveConversation(AiConversation conversation) async {
    ...
    for (var offset = 0; offset < conversation.messages.length; offset += 400) {
      final batch = _db.batch();
      final end =
          offset + 400 < conversation.messages.length
              ? offset + 400
              : conversation.messages.length;
      for (final message in conversation.messages.sublist(offset, end)) {
        batch.set(
  ...
  Future<void> saveMessage(String conversationId, AiChatMessage message) async {
    ...
    batch.update(reference, {'updatedAt': message.createdAt});
    await batch.commit();
  }
  ...
  Future<void> deleteMessage(String conversationId, String messageId) async {
    ...
    batch.update(reference, {'updatedAt': DateTime.now()});
    await batch.commit();
  }
